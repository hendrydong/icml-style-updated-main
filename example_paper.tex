...
% For the initial blind version submitted for review, use:
% \usepackage{icml2026}

% For preprint, use
% \usepackage[preprint]{icml2026}

% The example PDF is built in camera-ready/final mode:
...
% Camera-ready papers must include a conflict of interest disclosure near the
% end of the introduction. For example:
...
\begin{itemize}
  ...
  \item Page limit: Initial submissions must fit the main body of the paper to
        8 pages, excluding references and appendices; the space for the latter
        two is not limited in pages. Camera-ready papers may use up to 9 pages
        for the main body. The complete camera-ready PDF, including appendices,
        may not exceed 20MB.
  \item \textbf{Do not include author information or acknowledgements} in your
        initial submission.
  \item Your paper should be in \textbf{10 point Times font}.
  \item Use standard, embedded fonts whenever possible. Type-1 fonts are
        recommended for compatibility, but authors should follow the current
        camera-ready checker guidance for the final enforced checks.
...
For best portability, please use standard embedded fonts. Authors may inspect
their PDFs with tools such as \texttt{pdffonts} in Linux or
File/DocumentProperties/Fonts in Acrobat. Nonstandard fonts can sometimes come
from graphics files imported into the document.
...
Those using \texttt{latex} and \texttt{dvips} who need to improve font
embedding may find the following commands helpful:
...
results and supports more advanced features in the \texttt{microtype} package.
...
The complete camera-ready PDF, including any appendices, must be submitted as a
single file and may not exceed 20MB. There is no separate camera-ready
supplementary material upload; any appendix material that should appear with the
paper must be included in the camera-ready PDF.

Accepted authors must also complete the ICML Publishing Release, the Recording
Release and License, and the PMLR Publication Agreement. These are separate
requirements, and completing one does not replace the others. Uploaded consent,
release, or agreement PDFs should not exceed 10MB. Camera-ready authors may also be
asked to enter the five-letter paper checker code in the OpenReview
camera-ready form.
...
to be submitted as supplementary material via OpenReview during review. However,
keep in mind that an ICML paper should be self contained and should contain
sufficient detail for the reviewers to evaluate the work. In particular,
reviewers are not required to look at supplementary material when writing their
...
review. Instead, provide an anonymous URL or, for the review phase only, upload
the material as supplementary material into the OpenReview reviewing system.
Note that reviewers are not required to look at this material when writing their
review. For the camera-ready version, there is no separate supplementary
material upload; include appendix material in the single camera-ready PDF and
use URLs for software or data when appropriate.
...
Authors should follow the impact-statement requirements for their paper track.
For tracks that require an impact statement, include a statement of the
potential broader impact of the work, including its ethical aspects and future
societal consequences. This statement should be in an unnumbered section at the
end of the paper (co-located with Acknowledgements -- the two may appear in
either order, but both must be before References), and does not count toward the
paper page limit. Position paper track submissions should follow their
track-specific instructions and may omit this section if instructed to do so. In
many cases, where the ethical impacts and expected societal implications are
those that are well established when advancing the field of Machine Learning,
substantial discussion is not required, and a simple statement such as the
...
You can have as much text here as you want. For review submissions, the main
body must be at most $8$ pages long. For the camera-ready version, the main body
may be at most $9$ pages long. If you want, you can use an appendix like this
one.
